...
import '../security_guard/main_security_guard.dart';
class TeddyController extends FlareControls {
  ...
  void submitPassword(BuildContext ctx,int choice) {
    if (_password == "bears") {
      play("success");
      //Scaffold.of(ctx).showSnackBar(SnackBar(content: Text('Processing Data')));
      switch(choice)
      {
       case 1: Future.delayed(const Duration(seconds: 2),(){
        Navigator.of(ctx).pushAndRemoveUntil(MaterialPageRoute(builder: (_)=>admin2()),(_)=> false);
      }  );
        break;
         case 2: Future.delayed(const Duration(seconds: 2),(){
        Navigator.of(ctx).pushAndRemoveUntil(MaterialPageRoute(builder: (_)=>HomePage()),(_)=> false);
      }  );
        break;
         case 3: Future.delayed(const Duration(seconds: 2),(){
        Navigator.of(ctx).pushAndRemoveUntil(MaterialPageRoute(builder: (_)=>HomePage()),(_)=> false);
      }  );
        break;
         case 4: Future.delayed(const Duration(seconds: 2),(){
        Navigator.of(ctx).pushAndRemoveUntil(MaterialPageRoute(builder: (_)=>finale1()),(_)=> false);
      }  );
        break; 
